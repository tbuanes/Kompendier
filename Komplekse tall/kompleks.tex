...
\end{displaymath}
kan evalueres ved hjelp av delvis integrasjon, men det er en del jobb siden vi blant annet må gjennom to steg med delvis integrasjon. La oss derfor i stedet gå via kompleks representasjon av cosinus.\footnote{Dette eksempelet bruker en del av regnereglene  for komplekse som først blir presentert i kommende avsnitt. Det bør likevel gå greit å følge logikken i eksempelet, og du kan eventuelt vende tilbake til det senere også.}
\begin{align*}
	\int_0^\infty e^{-x}\cos x \dx&= \int_0^\infty e^{-x}\frac{e^{ix}+e^{-ix}}{2} \dx \\
	&= \frac12\int_0^\infty \left(e^{(i-1)x}+e^{-(i+1)x}\right)\dx \\
	&= \frac12\left.\left(\frac{e^{(i-1)x}}{i-1}-\frac{e^{-(i+1)x}}{i+1}\right)\right|_0^\infty
\end{align*}
\end{texample}
\end{comment}
...
\end{displaymath}
\begin{texample}
	La $z_1 = 1 - i$, $z_2 = 1$, $z_3 = 2i$ og $z_4 = 2e{-i\frac{\pi}{3}}$. Vi finner da
	\begin{align*}
		\bar{z} _1&= 1+i \\
		\bar{z}_2 &= 1 \\
		\bar{z}_3 &= -2i \\
		\bar{z}_4 &= 2e^{i\frac{\pi}{3}}
	\end{align*}
	Merk at et reellt tall ikke endres av komplekskonjugering ($\bar{z}_2 = z_2$), mens for et rent imaginært tall har komplekskonjugering samme effekt som å multiplisere med $-1$ ($\bar{z}_3 = -z_3$).
\end{texample}
...
\begin{displaymath}
	z = \frac{-2\pm\sqrt{2^2-4\cdot1\cdot2}}{2\cdot 1}= \frac{-2\pm\sqrt{-4}}{2} = \frac{-2\pm2\sqrt{-1}}{2}
\end{displaymath}
Siden svaret vi får fra $abc$-formelen inkluderer $\sqrt{-1}$ har vi ingen løsninger for reelle tall, men når vi tillater $z$ å være kompleks har vi to løsninger:
\begin{displaymath}
	z= \ans{-1 \pm i}
\end{displaymath}
siden $\pm\sqrt{-1} = \pm i$.
\end{texample}
...
\item $\sqrt[3]{2}e^{i\frac{\pi}{6}}$, $\sqrt[3]{2}e^{i\frac{5\pi}{6}}$, \\$\sqrt[3]{2}e^{i\frac{9\pi}{6}}$
